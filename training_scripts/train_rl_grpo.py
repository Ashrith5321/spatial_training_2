...
def main(cfg: RLConfig):
    ...
    from utils.rl_core import collate_trajectories,grouped_rollouts
    ...
    try:
        ...
        for global_cycle in range(num_rollouts):
            ...
            #randomly sample the episode indices according to n_rollout:
            # episode_indices = np.random.randint(0, bootstrapper.typed_cfg.training.total_optimization_steps, size=bootstrapper.typed_cfg.training.rl_config.n_rollout)
            episode_indices = np.random.randint(0, 2000, size=bootstrapper.typed_cfg.training.rl_config.n_rollout)
            # episode_indices=[3,5,2]
            ...
            rollout_list,result_list,log_list = grouped_rollouts(sims,trainers,episode_indices,bootstrapper.typed_cfg.training.rl_config.group_size) #
            count_per_indices = defaultdict(int)
            for result in result_list:
                count_per_indices[result['episode_idx']] += 1
            index = [result['episode_idx'] for result in result_list]
            print(count_per_indices)
            ...
            adv_tuple = advantage_estimator_fn(
                ...
                index = traj_batch['episode_idx'][:,0].numpy() #default GRPO uses flat.
            ...
            traj_batch = traj_batch[-bootstrapper.typed_cfg.training.rl_config.n_rollout*bootstrapper.typed_cfg.training.rl_config.group_size:] # only train on most recent.
            ...
            n_rollout = bootstrapper.typed_cfg.training.rl_config.n_rollout*bootstrapper.typed_cfg.training.rl_config.group_size
            for i in range(bootstrapper.typed_cfg.training.rl_config.n_epoch-1):
                print(f"epoch {i}")
                training_futures = []
                perm_indices = np.random.permutation(n_rollout) # shuffle
