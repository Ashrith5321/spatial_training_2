...
class OVONDatasetV1(PointNavDatasetV1):
    ...
    def from_json(self, json_str: str, scenes_dir: Optional[str] = None) -> None:
        ...
        # 1. Gather all unique category strings from the loaded episodes
        cats = sorted({ep.object_category for ep in self.episodes})
        
        # 2. Create the arbitrary 1-to-1 mapping (Integer <-> String)
        cat2id = OrderedDict((c, i) for i, c in enumerate(cats))

        # 3. Patch the attributes standard Habitat sensors look for
        # This allows the 'ObjectGoal' sensor to find the integer ID for a string goal
        self.category_to_task_category_id = cat2id
        
        # This allows metrics to map predictions back to ground truth
        self.category_to_scene_annotation_category_id = cat2id
